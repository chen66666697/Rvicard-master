...
static inline uint32_t rgb565_to_fb32(uint16_t color16)
{
    ...
    // 小端内存顺序 ARGB8888 (0x00RRGGBB)
    return (r << 16) | (g << 8) | b; 
}

static int lcd_fb_display_px(uint16_t color16, int x, int y)
{
    uint32_t *pen = (uint32_t *)(fb_mem + y * line_width + x * px_width);
    *pen = rgb565_to_fb32(color16);
    return 0;
}
